...
async def _resolve_model() -> str:
    configured = await runtime_settings.get_openai_model_tts()
    if configured:
        return configured
    return (
...
async def _resolve_default_voice() -> str:
    configured = await runtime_settings.get_openai_tts_voice()
    if configured:
        return configured
    return os.getenv("OPENAI_TTS_VOICE", "alloy")

async def _map_voice(logical: str) -> str:
    return logical or await _resolve_default_voice()
